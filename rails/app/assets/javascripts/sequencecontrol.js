	...
	decodeFromString : function(str) {
		...
		while (( openIndex = StringUtil.indexOfAny(str, ['<', "\\"], lastMatchIndex)) !== -1) {
			...
			if (str.charAt(openIndex) === "\\") {
				lastMatchIndex++;
				// das Zeichen nach dem Escape-Zeichen überspringen
				continue;
			}
...
var SequenceControl = ( function() {
		...
		var addSpecialCharacterPopover = function(component) {
			...
			specialCharacterButton.popover({
				my : 'center top',
				at : 'center bottom',
				popover : specialCharacterPopover,
			...
			// Position bleibt
			specialCharacterButton.mousedown(function(e) {
				e.preventDefault();
			});
		};
		...
		var destroy = function() {
			...
			if (confirm('Wollen Sie wirklich die ausgewählte Sequenz „' + getSelectedSequenceName() + '“ löschen?')) {
				...
				FlashMessage.error("Löschen abgebrochen");
			}
		};
		...
		var init = function() {
			...
			function preventDefaultProxy(fun) {
				return function(e) {
					fun();
					e.preventDefault();
				};
